...
import { Check, Network, ArrowLeft, CreditCard, Loader2 } from "lucide-react";
...
export default function SubscribeSchemaConceptuelPage() {
  ...
  async function handleTrial() {
    ...
    setError("");
    try {
      await apiRequest("POST", `/api/schema-conceptuel-trial/activate?token=${token}`, {});
  ...
  async function handleSubscribe() {
    ...
    setError("");
    try {
      const res = await apiRequest("POST", `/api/stripe/create-checkout?token=${token}`, { plan: "schema_conceptuel_annual" });
    ...
    <div className="v2-page min-h-screen flex flex-col bg-background">
      ...
      <div className="flex-1 px-4 py-12 max-w-lg mx-auto w-full">
        ...
        <div className="flex items-center gap-3 mb-8">
          ...
          <div>
            <h1 className="text-xl font-extrabold text-foreground">Schéma Conceptuel — IEM + PG</h1>
            <p className="text-xs text-muted-foreground">Source › Vecteur › Cible — Licence annuelle</p>
          </div>
        </div>
        ...
        <div className="rounded-xl border-2 p-6 shadow-md relative bg-white" style={{ borderColor: "#2ecc71" }}>
          ...
          <ul className="space-y-2 mb-6 text-sm">
            <PricingItem>Schéma SVG interactif Source › Vecteur › Cible</PricingItem>
            <PricingItem>Application IEM (ANSES 2018) + Plan de Gestion (MEEM/MTES 2017)</PricingItem>
            <PricingItem>Connexion native EQRS V31.05 + ECOTOX V8</PricingItem>
            <PricingItem>Synthèse cumulée par cible</PricingItem>
            <PricingItem>Export PNG / PDF du modèle conceptuel</PricingItem>
          ...
          {!user && (
            <p className="text-center text-xs text-muted-foreground mt-3">
              ...
              <button
                ...
                onClick={() => { localStorage.setItem("pending_plan", "schema_conceptuel_annual"); window.location.hash = "#/login"; }}
              >
                Se connecter
              </button>
            </p>
